...
export default function WishlistPage() {
  ...
  const moveAllToCart = () => {
    const availableItems = wishlistItems.filter(i => i.inStock !== false);
    if (availableItems.length === 0) {
      alert("Không có sản phẩm nào còn hàng để chuyển!");
      return;
    }
    availableItems.forEach(prod => handleMoveToCart(prod));
    alert(`Đã thêm ${availableItems.length} sản phẩm vào giỏ hàng!`);
  };
    ...
    return (
      <div className="bg-white min-h-[70vh] flex flex-col items-center justify-center font-body px-4 mt-[72px]">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <Link to="/login" className="flex items-center justify-center gap-2 px-8 py-3.5 bg-[#DB4444] text-white font-medium rounded-sm hover:bg-red-600 transition-colors shadow-sm">
            <LogIn size={18} /> Đăng nhập ngay
          </Link>
          <Link to="/danh-muc" className="flex items-center justify-center gap-2 px-8 py-3.5 bg-white border border-gray-200 text-gray-700 font-medium rounded-sm hover:border-[#DB4444] hover:text-[#DB4444] transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {wishlistItems.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-medium text-gray-900 mb-2 font-display">Danh sách yêu thích trống</h3>
            <p className="text-gray-500 mb-8">Bạn chưa lưu sản phẩm nào vào danh sách yêu thích.</p>
          ...
          <>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
              ...
              <button onClick={moveAllToCart} className="px-6 py-3 border border-gray-300 text-gray-900 font-medium rounded-sm hover:bg-gray-50 transition-colors text-sm w-full sm:w-auto">
                Chuyển tất cả vào Giỏ hàng
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-8 sm:gap-x-5 lg:gap-x-6">
              {wishlistItems.map((prod) => (
                <div key={prod.id} className="group relative flex flex-col h-full min-w-0">
                  {prod.oldPrice && prod.inStock !== false && (
                  ...
                  <div className="relative bg-gray-50 aspect-[4/5] rounded-sm overflow-hidden mb-3">
                    <img
                      ...
                      className={`w-full h-full object-cover mix-blend-multiply transition-transform duration-500 ${
                        prod.inStock !== false ? "group-hover:scale-105" : "opacity-60"
                      }`}
                    />

                    {prod.inStock === false && (
                      <div className="absolute inset-0 bg-white/60 flex items-center justify-center">
                        <span className="bg-gray-700 text-white text-xs font-semibold px-3 py-1.5 rounded-sm">Hết hàng</span>
                      </div>
                    )}
                    ...
                    {prod.inStock !== false && (
                      <button 
                        ...
                        className="hidden lg:flex absolute bottom-0 left-0 right-0 bg-black text-white font-medium py-2.5 text-xs items-center justify-center gap-2 translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 hover:bg-[#DB4444]"
                      >
                  ...
                  <div className="flex flex-col flex-1 min-w-0">
                    ...
                    <Link to={`/san-pham/${prod.id}`} className="text-sm font-medium text-gray-900 leading-snug mb-1 hover:text-[#DB4444] transition-colors line-clamp-2">
                      {prod.name}
                    </Link>
                    <div className="flex items-center gap-2 flex-wrap mt-auto pt-1">
                      <span className={`font-semibold text-sm ${prod.inStock !== false ? "text-[#DB4444]" : "text-gray-400"}`}>
                    ...
                    <button
                      ...
                      className={`lg:hidden mt-3 w-full py-2.5 text-xs font-medium rounded-sm flex items-center justify-center gap-2 transition-colors ${
                        prod.inStock !== false ? "bg-black text-white hover:bg-[#DB4444]" : "bg-gray-100 text-gray-400 cursor-not-allowed"
                      }`}
                    >
                      <ShoppingCart size={13} />
                      {prod.inStock !== false ? "Thêm vào giỏ" : "Hết hàng"}
                    </button>
                  </div>
                </div>
              ))}
